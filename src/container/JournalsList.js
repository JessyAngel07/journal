...
class JournalsList extends Component {
    ...
    addPage = async () => {
        ...
        await db.journalsList.add({ heading: 'untitled', date: Date().slice(4, 15) })
    ...
    onHandleInput = async (i) => {
        ...
        try {
            if (pass?.trim() !== '') {
                ...
                if (updated) {
                    console.log(`Heading updated ${i} successfully!!`)
                }
                else {
                    console.log(`not found id ${i}`)
                }
            }
        }
        catch (error) {
    ...
    render() {
        // console.log(this.state.cards, 'get', Date().slice(4, 21))
        return (
            <div>
                <NavHeader />
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap' }} >
                    {this.state.cards?.map(i =>
                        <div style={{ border: '0px solid', width: '250px', backgroundColor: '#0d0131', borderRadius: '12px', margin: '25px',
